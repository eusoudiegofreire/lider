...
| 2026-07-31 | Foto da equipe removida do Hero (duas tentativas com máscara de gradiente na borda esquerda, nenhuma resolveu a leitura de "mancha diagonal amadora" — o problema era a diferença de luminância entre o chão claro e a porta escura da loja, não a geometria da máscara). Hero volta a ser só texto, coluna única, `max-w-2xl` dentro da seção — o espaço sobrando é intencional, não um elemento gráfico de preenchimento. Foto entra numa seção nova, "Quem Somos", sem máscara/clip-path — `aspect-[4/5]` bate a proporção nativa do arquivo, então `object-cover` não corta nada | Depois de dois ajustes de curva/luminância na máscara, o resultado ainda lia como "colado por cima". Melhor não forçar a foto num formato (sangria de metade da tela) que não combina com a proporção retrato do arquivo real, do que insistir num tratamento que não fecha |
